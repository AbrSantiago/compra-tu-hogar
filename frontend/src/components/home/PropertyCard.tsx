  ...
  const handleFavoriteClick = async (e: React.MouseEvent) => {
    ...
    try {
      if (props.initialIsFavorite) {
        await clientService.removeFavorite(clientId, props.id);
      } else {
        await clientService.addFavorite(clientId, props.id);
      }
      ...
      >
          ...
          {props.userRole === 'client' && (
            ...
            >
              <svg
                ...
                viewBox="0 0 24 24"
                fill={props.initialIsFavorite ? "currentColor" : "none"}
                stroke="currentColor"
                strokeWidth={2}
        ...
        <div className="px-1 space-y-1">
          ...
          <p className="text-xs text-slate-400 truncate">
            {typeMap[props.type]}
            {props.characteristics && ` • ${props.characteristics}`}
          </p>
