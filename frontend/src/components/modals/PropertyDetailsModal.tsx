        ...
        <div className="p-6 space-y-6 flex-1">
          ...
          <div>
            <div className="flex justify-between items-start">
              ...
              <span className="text-xl font-bold text-blue-600">USD {property.price.toLocaleString('es-AR')}</span>
            </div>
          ...
          <div>
            <h3 className="text-lg font-bold text-slate-900 mb-4 flex items-center gap-2">
              ...
              {property.averageRating != null && (
                <span className="text-sm font-normal text-slate-500 bg-slate-100 px-2 py-0.5 rounded-full">
                  ★ {property.averageRating.toFixed(1)} / 10
                </span>
              )}
            </h3>
            ...
            <div className="space-y-4 mb-6">
              {property.reviews.length === 0 ? (
                <p className="text-sm text-slate-500 italic">Aún no hay reseñas para esta propiedad.</p>
              ) : (
                property.reviews.map(review => {
                  ...
                  return (
                    <div
                      key={review.id}
                      className={`p-3 rounded-xl border transition-all ${isMyReview ? 'bg-blue-50/60 border-blue-200' : 'bg-slate-50 border-slate-100'}`}
                    >
